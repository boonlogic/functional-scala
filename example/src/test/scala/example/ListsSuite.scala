 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of more numbers") {
    assert(Lists.sum(List(1,2,2,10)) === 15)
  }
  ...
  test("max of numbers including negative numbers") {
    assert(Lists.max(List(3, -7, 2)) === 3)
  }

  test("max of singleton number") {
    assert(Lists.max(List(3)) === 3)
  }
